...
interface Story {
  ...
  inwardLinks: string | null;
  outwardLinks: string | null;
  team: string | null;
...
const STORAGE_KEY = 'stories-column-config';
...
const Stories: React.FC = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['stories'],
    queryFn: async () => {
      const response = await apiClient.get<Story[]>('/api/data/stories');
      const stories = response.data || [];
      // Debug: Log first story's sprints to verify data structure
      if (stories.length > 0) {
        console.log('First story sprints:', stories[0].sprints, 'Type:', typeof stories[0].sprints);
      }
      return stories;
    },
  });
  ...
  const handleFilePreview = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/data/stories/preview`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/data/stories/upload`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.success) {
        toast.success(`Successfully imported ${result.data.processed} stories`);
        setFile(null);
        setPreviewData(null);
        refetch();
      } else {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-gray-500">No stories found. Upload a CSV file to get started.</div>
        ) : (
                ...
                {paginatedStories.map((story) => (
                  <tr key={story.id} className="hover:bg-gray-50">
                    {visibleColumns.map((col) => {
                      ...
                      const value = (story as any)[col.key];
                      ...
                      switch (col.key) {
                        case 'issueKey':
                          content = <span className="text-sm font-medium text-gray-900">{story.issueKey}</span>;
                          break;
                        case 'summary':
                          content = <span className="text-sm text-gray-900 max-w-md truncate">{story.summary}</span>;
                          break;
                        case 'status':
                          content = (
                      ...
                      const isLongText = ['summary', 'description', 'checklistText', 'comments'].includes(col.key);
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, stories.length)}</span> of{' '}
                  <span className="font-medium">{stories.length}</span> results
                </p>
              </div>
              <div>
