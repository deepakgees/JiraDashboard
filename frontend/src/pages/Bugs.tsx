...
interface Bug {
  ...
  affectedHardware: string | null;
  affectedServices: string | null;
  incidentId: string | null;
  incidentActive: string | null;
...
const DEFAULT_COLUMNS: ColumnConfig[] = [
  ...
  { key: 'resolution', label: 'Resolution', visible: true, order: 5 },
  ...
  { key: 'resolved', label: 'Resolved', visible: true, order: 24 },
  ...
  { key: 'affectedArtifacts', label: 'Affected Artifacts', visible: false, order: 52 },
  { key: 'affectedHardware', label: 'Affected Hardware', visible: false, order: 53 },
  { key: 'affectedServices', label: 'Affected Services', visible: false, order: 54 },
  { key: 'incidentId', label: 'Incident ID', visible: false, order: 55 },
  { key: 'incidentActive', label: 'Incident Active', visible: false, order: 56 },
  { key: 'majorIncident', label: 'Major Incident', visible: false, order: 57 },
  { key: 'vulnerability', label: 'Vulnerability', visible: false, order: 58 },
  { key: 'dateApprovalDone', label: 'Date Approval Done', visible: false, order: 59 },
  { key: 'dateConceptDone', label: 'Date Concept Done', visible: false, order: 60 },
  { key: 'dateDevelopmentDone', label: 'Date Development Done', visible: false, order: 61 },
  { key: 'dateTestDone', label: 'Date Test Done', visible: false, order: 62 },
  { key: 'dateGoLive', label: 'Date Go Live', visible: false, order: 63 },
  { key: 'dateApprovalProductionEnv', label: 'Date Approval Production Env', visible: false, order: 64 },
  { key: 'dateApprovalStagingEnv', label: 'Date Approval Staging Env', visible: false, order: 65 },
  { key: 'dateApprovalTestEnv', label: 'Date Approval Test Env', visible: false, order: 66 },
  { key: 'slaStartDate', label: 'SLA Start Date', visible: false, order: 67 },
  { key: 'slaTargetDate', label: 'SLA Target Date', visible: false, order: 68 },
  { key: 'timeToFirstResponse', label: 'Time to First Response', visible: false, order: 69 },
  { key: 'timeToResolution', label: 'Time to Resolution', visible: false, order: 70 },
  { key: 'dateOfFirstResponse', label: 'Date of First Response', visible: false, order: 71 },
  { key: 'timeInStatus', label: 'Time in Status', visible: false, order: 72 },
  { key: 'satisfactionRating', label: 'Satisfaction Rating', visible: false, order: 73 },
  { key: 'satisfactionDate', label: 'Satisfaction Date', visible: false, order: 74 },
  { key: 'sentiment', label: 'Sentiment', visible: false, order: 75 },
  { key: 'solutionApproach', label: 'Solution Approach', visible: false, order: 76 },
  { key: 'solutionProposal', label: 'Solution Proposal', visible: false, order: 77 },
  { key: 'solutionQuality', label: 'Solution Quality', visible: false, order: 78 },
  { key: 'attachments', label: 'Attachments', visible: false, order: 79 },
  { key: 'securityLevel', label: 'Security Level', visible: false, order: 80 },
  { key: 'votes', label: 'Votes', visible: false, order: 81 },
];

const STORAGE_KEY = 'bugs-column-config';
...
const Bugs: React.FC = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['bugs'],
    queryFn: async () => {
      const response = await apiClient.get<Bug[]>('/api/data/bugs');
  ...
  const handleFilePreview = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/data/bugs/preview`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/data/bugs/upload`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.success) {
        toast.success(`Successfully imported ${result.data.processed} bugs`);
        setFile(null);
        setPreviewData(null);
        refetch();
      } else {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold text-gray-900">Bugs (New Dev)</h1>
      ...
      <div className="bg-white shadow rounded-lg overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-gray-500">No bugs found. Upload a CSV file to get started.</div>
        ) : (
                ...
                {paginatedBugs.map((bug) => (
                  <tr key={bug.id} className="hover:bg-gray-50">
                    {visibleColumns.map((col) => {
                      ...
                      const value = (bug as any)[col.key];
                      ...
                      switch (col.key) {
                        case 'issueKey':
                          content = <span className="text-sm font-medium text-gray-900">{bug.issueKey}</span>;
                          break;
                        case 'summary':
                          content = <span className="text-sm text-gray-900 max-w-md truncate" title={bug.summary}>{bug.summary}</span>;
                          break;
                        case 'status':
                          content = (
                      ...
                      const isLongText = ['summary', 'description', 'checklistText', 'comments', 'environment', 'solutionApproach', 'solutionProposal'].includes(col.key);
        ...
        {/* Pagination Controls */}
        {!isLoading && bugs.length > 0 && totalPages > 1 && (
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{Math.min(endIndex, bugs.length)}</span> of{' '}
                  <span className="font-medium">{bugs.length}</span> results
                </p>
              </div>
              <div>
